...
// Physics Constants - Precision tuned for organic but stable tree growth
...
const App: React.FC = () => {
  ...
  const zoomRef = useRef(zoom); // Use ref to track zoom for stable calculations in events
  ...
  const animate = useCallback(() => {
    setNodes(prevNodes => {
      ...
      newNodes.forEach(node => {
        if (!vels[node.id]) vels[node.id] = { vx: 0, vy: 0 };
      });
      ...
      return newNodes.map(node => {
        ...
        vels[node.id].vx = vx * DAMPING;
        vels[node.id].vy = vy * DAMPING;
  ...
  const updateZoom = useCallback((newZoom: number, mouseX?: number, mouseY?: number) => {
    ...
    // Always use current real zoom for pivot calculations to prevent "teleporting"
    ...
    // Apply scroll adjustment on the next frame to match the scale re-render
  ...
  const centerOn = useCallback((x: number, y: number, behavior: ScrollBehavior = 'smooth') => {
    ...
    const currentZoom = zoomRef.current;
    const targetLeft = (x * currentZoom) - clientWidth / 2;
    const targetTop = (y * currentZoom) - clientHeight / 2;
  ...
  const jumpToNode = useCallback((node: ThoughtNode, adjustZoom = false) => {
    ...
    setTimeout(() => {
      if (containerRef.current) {
        ...
        // Use manual centering calculation to ensure it uses targetZoom
        const { clientWidth, clientHeight: cHeight } = containerRef.current;
  ...
  const startJourney = async (input: string) => {
    ...
    try {
      ...
      setNodes(prev => prev.map(n => n.id === rootId ? { ...n, isLoading: false, description: "Connection limit reached. Please try again." } : n));
    }
  };
      ...
      setNodes(prev => {
        const newNodes: ThoughtNode[] = topics.map((item, index) => {
          ...
          return {
            ...
            parentId, 
            level: nextLevel, 
            position: { x: position.x + offset, y: position.y + spacingY }, 
            isExpanded: false, 
            isLoading: false, 
            isNew: true, 
            sources: sources,
            path: [...currentPath, item.topic]
          };
        });
  ...
  const visibleNodesAndEdges = useMemo(() => {
    ...
    const visibleNodes = nodes.filter(n => {
      if (!isNodeEffectivelyVisible(n, nodes)) return false;
      return n.position.x > vX && n.position.x < vX + vW && n.position.y > vY && n.position.y < vY + vH;
    });
    ...
    const visibleEdges = edges.filter(e => {
      ...
      return (from.position.x > vX && from.position.x < vX + vW && from.position.y > vY && from.position.y < vY + vH) ||
             (to.position.x > vX && to.position.x < vX + vW && to.position.y > vY && to.position.y < vY + vH);
    });
        ...
        <>
              ...
              <div className={`flex items-center bg-white dark:bg-gray-950 border-2 rounded-2xl shadow-xl overflow-hidden h-12 md:h-14 w-full transition-all duration-500 ${isSearchExpanded ? 'border-blue-500 ring-4 ring-blue-500/10' : 'border-slate-200 dark:border-gray-800'}`}>
                ...
                {isSearchExpanded && (
                  <input 
                    ...
                    className="w-full bg-transparent border-none text-[13px] font-bold focus:ring-0 outline-none text-slate-900 dark:text-white pr-4" 
                    value={searchQuery} 
                ...
                <div className="mt-2 w-full bg-white dark:bg-gray-950 border border-slate-200 dark:border-gray-800 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-2xl animate-menu-pop flex flex-col z-[4000]">
                  {searchResults.length > 0 ? (
                    searchResults.map(result => (
                      <button
                        key={result.id}
                        onClick={(e) => { e.stopPropagation(); jumpToNode(result, true); setSearchQuery(""); setIsSearchExpanded(false); }}
                        className="w-full px-5 py-3.5 text-left hover:bg-slate-50 dark:hover:bg-white/5 transition-colors flex items-center justify-between border-b border-slate-100 dark:border-gray-900 last:border-none group"
                      >
                    ...
                    <div className="px-5 py-6 text-center text-slate-400 text-[11px] font-bold uppercase tracking-widest">
                ...
                return (
                  <div key={node.id} className="transition-opacity duration-500" style={{ opacity: isOtherNodeSelected ? 0.2 : 1 }}>
                    <NodeItem 
                      ...
                      onBranch={() => expandNode(node.id, node.label, node.position, node.level, node.path, false)} 
                      onExploreFurther={() => expandNode(node.id, node.label, node.position, node.level, node.path, true)} 
                      ...
                      onHover={() => {}}
                      onDrag={(id, x, y) => setNodes(prev => {
                        const next = prev.map(n => n.id === id ? { ...n, position: { x, y } } : n);
                        if (nodeVelocities.current[id]) {
                          nodeVelocities.current[id].vx = 0;
                          nodeVelocities.current[id].vy = 0;
                        }
                        return next;
                      })} 
                      theme={theme} 
          ...
          <div className="fixed bottom-6 md:bottom-8 left-6 md:left-8 z-[3000] flex flex-col items-start space-y-3 pointer-events-auto">
            {isOptionsMenuOpen && !isEditMode && (
              <div className="bg-white dark:bg-gray-950 rounded-[2rem] border border-slate-200 dark:border-gray-800 p-2 shadow-3xl w-48 animate-menu-pop flex flex-col space-y-1" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={(e) => { e.stopPropagation(); setIsStarted(false); setNodes([]); setEdges([]); nodeVelocities.current = {}; }} className="flex items-center space-x-3 px-4 py-3 rounded-2xl text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all">
            ...
            <div className="flex flex-col space-y-3">
              <button 
                onClick={(e) => { e.stopPropagation(); if (nodes.length > 0) centerOn(nodes[0].position.x, nodes[0].position.y); setIsOptionsMenuOpen(false); }} 
          ...
          <div className="fixed bottom-6 md:bottom-8 right-6 md:right-8 z-[3000] flex flex-col items-end space-y-3 pointer-events-auto">
            ...
            <button onClick={(e) => { e.stopPropagation(); setIsMinimapOpen(!isMinimapOpen); }} className={`w-14 h-14 flex items-center justify-center rounded-2xl transition-all shadow-2xl border ${isMinimapOpen ? 'bg-blue-600 border-blue-400 text-white' : 'bg-white dark:bg-gray-950 border-slate-200 dark:border-gray-800 text-slate-500 hover:scale-105'}`}>
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
            </button>
          </div>
