...
export const Minimap: React.FC<MinimapProps> = ({ nodes, canvasSize, viewport, isOpen, isMobile }) => {
  ...
  return (
    ...
    >
      <div className="relative w-full h-full">
        ...
        <div 
          className="absolute border-2 border-blue-500 bg-blue-500/10 rounded-sm transition-all duration-300 z-10"
          style={{
        ...
        {nodes.map(node => (
          <div 
            key={node.id}
            className={`absolute rounded-full transition-all duration-500 ${node.level === 0 ? 'bg-blue-600 w-1.5 h-1.5' : 'bg-slate-400 dark:bg-gray-600 w-1 h-1'}`}
            style={{
